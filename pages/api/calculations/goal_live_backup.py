...
def fillDates(goal, yearsUntilMortality, birthDate):
    ...
    if (when == "date"):
        ...
        print("-- TYPE CHECKING dateRange --")
        print(type(goal.get("dateRange")))
        print(type(goal.get("dateRange")[0]))
...
def calculateGoalTables(goals, portfolioAmount, yearsUntilMortality, assumptions, clientData, actualRateTable, portfolioTerms):
    ...
    for i in range(1, len(goals)+1):
        ...
        print("6")
        table_data = list(zip(startDates, endDates, yearFloats, rates, presentValues, pfNames, portfolioAmounts, portfolioAmountsRemaining, funded, fundedAmount, monthlySavingsRequired))
        print("7")
...
def calculateGoalsAllocations(assumptions, behaviourGrowthRate, goals, portfolios, clientData, portfolioValue):   
    ...
    return goalTablesJSON, goalOverallFundedPercent
