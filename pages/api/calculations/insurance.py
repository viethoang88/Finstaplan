...
def getDataForDataFrameSingle(client_data, role, name):
    ...
    return list(map(lambda x: addNameToDictionary(x, name), pAll))

def getDataForDependents(client_data, role):        
   data = []
   dependentsList = client_data[role]       
   for idx, person in enumerate(dependentsList, start=1):
       ...
       _all = expenses + liabilities + assets + incomes 
...
def getDefaultRate():
    return assumptions["growthRate"]["CPI"] / 100
...
def extractDataAndReturnDataFrame(client_data):
    primaryData = getDataForDataFrameSingle(clientData, "primary", client_data["primary"].get("firstName", "primary"))
    partnerData = []
    childrenData = []
    dependentsData = []
    jointDependentsData = []
    partnerDependentsData = [] 
    jointData = getDataForDataFrameSingle(clientData, "joint", "joint")
...
def getAggExpenses(expenses, requirements, assumptions, clientData):
    if (clientData["hasChildren"]): 
        childExpenses = getChildExpenses(expenses[expenses["class"] == "childExpense"], assumptions, assumptions["calculatedAssumptions"]["numYearsToSupportChildren"])
    ...
    otherExpenses = expenses[expenses["class"] != "childExpense"]
    partner = None 
    ...
    if (clientData["hasPartner"]):
        ...
        partner = clientData["partner"]["firstName"]
...
def calculateInsuranceRequirements(client, yearsUntilMortality, yearsUntilRetirement, assumptions, assets, incomes, expenses, liabilities, clientData, philosophies):
    ...
    portfolioGrowthRate = clientData["riskProfile"].get("portfolioRate") or 3.5
    ...
    _childExpenses = expenses[expenses["class"] == "childExpense"]
...
def insurance_calc(c=None, p=None, a=None):
    ...
    [1].__contains__(1)
    if (c == None): 
        return initialize_as_fn(clientData, philosophies, assumptions)
    else:
...
def main():
    ...
    try: 
        if (sys.argv[1]): 
            print(sys.argv)
            clientData = json.loads(sys.argv[1])
            philosophies = json.loads(sys.argv[2])
            assumptions = json.loads(sys.argv[3])
            initialize(clientData, philosophies, assumptions)
        else:
...
netPerYear = res["primary"]["netPerYear"]
taxPaid =  res["primary"]["taxPaid"]
netAmountRequiredAfterTax = res["primary"]["netAmountRequiredAfterTax"]

aggLiabilitiesByCategory = res["primary"]["aggLiabilitiesByCategory"]
liabilitiesPerYear = res["primary"]["liabilitiesPerYear"]
offsetsSumPerYear = res["primary"]["offsetsSumPerYear"]

incomesByCategorySumNpv =  res["primary"]["incomesByCategorySumNpv"]
assetsByCategorySumNpv =  res["primary"]["assetsByCategorySumNpv"]
minimumCoveragRequirements = res["primary"]["minimumCoverageRequirements"]

# WRONG 
expensesSumNpvPerYear = res["primary"]["expensesSumNpvPerYear"]
incomeRequirementsPerYear =  res["primary"]["incomeRequirementsPerYear"]
expensesByCategorySumNpv =  res["primary"]["expensesByCategorySumNpv"]
expensesPerYear =  res["primary"]["expensesPerYear"]
