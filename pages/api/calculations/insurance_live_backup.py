...
def getPercentsOnStepped(totals, percentOnStepped, numYearsOnLevel, transitionFromLevelToStepped):
    pStepped = { k: list(v.values())[0] 
                    for k, v in percentOnStepped.to_dict().items() 
                }    
    transitionFromLevelToStepped = { k: list(v.values())[0] 
                    for k, v in transitionFromLevelToStepped.to_dict().items() 
                }    
    stepped = {
        "category": "Stepped",
        }
    ...
    for k,v in transitionFromLevelToStepped.items():
        stepped[k] = v * pStepped[k] / 100 * totals[k]
        level[k] = totals[k] - stepped[k]
    print("------- PERCENTS ON STEPPED ----")
    print(stepped)
    print(level)
    print(totals)
    return stepped, level
    

def buildSteppedVsLevelTable(netPerYearDict, minimumCoverageRequirementsDict, steppedVsLevel):
    print("--- BUILDING S V L table ---")
    transitionFromLevelToStepped = steppedVsLevel[steppedVsLevel.type == "transitionFromLevelToStepped"][["life","tpd","trauma", "ip"]]
    numYearsOnLevel = steppedVsLevel[steppedVsLevel.type == "numYearsOnLevel"][["life","tpd","trauma", "ip"]]
    percentOnStepped = steppedVsLevel[steppedVsLevel.type == "percentOnStepped"][["life","tpd","trauma", "ip"]]
    print(netPerYearDict)
    print(minimumCoverageRequirementsDict)
    print(transitionFromLevelToStepped)
    print(numYearsOnLevel)
    print(percentOnStepped)
    life = max(netPerYearDict["life"], minimumCoverageRequirementsDict["life"])
    tpd = max(netPerYearDict["tpd"], minimumCoverageRequirementsDict["tpd"])
    trauma = max(netPerYearDict["trauma"], minimumCoverageRequirementsDict["trauma"])
    totals = { "life": life, "tpd": tpd, "trauma": trauma, "ip": 0}
    stepped, level = getPercentsOnStepped(totals, percentOnStepped, numYearsOnLevel, transitionFromLevelToStepped)
    table = {
        "stepped": stepped,
        "level": level,
        "total": totals,
    ...
    print(table)
    return table
    #pOnStepped = { "life": lifeStepped, "tpd": tpdStepped, "trauma": traumaStepped }
...
def calculateInsuranceRequirements(client, yearsUntilMortality, yearsUntilRetirement, assumptions, assets, incomes, expenses, liabilities, clientData, philosophies):
    ...
    ## Philosophies:   
    ## - NOTE TO SELF: percentages are in decimal form requiring / 100.
    ##   - CURRENTLY: all offsets are percentages 
    ##   - CURRENTLY: NOT all requirements are percentages
    
    requirements = pd.DataFrame.from_dict(philosophies["requirements"], orient="columns")
    #print("--- REQUIREMENTS ---")
    #print(requirements)
    requirementsNonPercentItems = ["medicalRequirements", "extraHelpRequired", "replacementIncomeNumMonths", "changeOfHousingCar", "partnerTimeOffWorkInMonths", "taxOnLumpSumInsurancePayment"]
    mask = requirements.apply(lambda x: x.type not in requirementsNonPercentItems, axis=1)
    #print(mask)
    requirements.loc[mask, ["life", "tpd", "trauma"]] = requirements.loc[mask, ["life", "tpd", "trauma"]] / 100
    #requirementsPercentItems = requirements.loc[requirements.apply(lambda x: x.type not in requirementsNonPercentItems, axis=1)]
    #requirementsPercentItems[["life", "trauma", "tpd"]].loc[:, :] = requirementsPercentItems[["life", "trauma", "tpd"]].loc[:, :] / 100
    #requirements.update(requirementsPercentItems)
    
    offsets = pd.DataFrame.from_dict(philosophies["offsets"], orient="columns")
    offsets[["life", "trauma", "tpd"]] = offsets[["life", "trauma", "tpd"]] / 100
    #print("--- OFFSETS ---")
    #print(offsets)
    ...
    netPerYearDict = { "life": netPerYear.iloc[0]["life"], "tpd": netPerYear.iloc[0]["tpd"], "trauma": netPerYear.iloc[0]["trauma"] }
    print("--- NET PER YEAR DICT ---")
    print(netPerYearDict)

    
    minimumCoverageRequirementsDict = { k: list(v.values())[0] 
                                           for k, v in minimumCoverageRequirements.to_dict().items() 
                                           }    
    print("--- MINIMUM COVERAGE REQUIREMENTS ---")
    print(minimumCoverageRequirements)
    print(minimumCoverageRequirementsDict)
    
    steppedVsLevelTable = buildSteppedVsLevelTable(netPerYearDict, minimumCoverageRequirementsDict, steppedVsLevel)
    # TODO: 
    ...
    return {
        EXTRA STUFF FOR DEBUGGING:
            "debugging": {
                ...
                "offsets": offsets,
                "steppedVsLevel": steppedVsLevel,
                'aggLiabilitiesDict': aggLiabilities.to_dict(),
                'aggLiabilities': aggLiabilities,
                'assetOffsets': assetOffsets,
                'expensesByYear': expensesByYear, 
                'childExpenses': childExpenses,
                'resDf3': resDf3, 
                'sumPerYear3': sumPerYear3,
                'assetSumPerYear': assetSumPerYear,
                'incomeOffsets': incomeOffsets,
                'partnerIncomes': partnerIncomes,
                'incomesSumNpvPerYear': incomesSumNpvPerYear,
                'incomesByCategorySumNpv': incomesByCategorySumNpv,
                'incomesSumNpvPerYear': incomesSumNpvPerYear,
                'offsetsSumNpvPerYear': offsetsSumNpvPerYear,
                'expensesByCategorySumNpv': expensesByCategorySumNpv,
                'assetsByCategorySumNpv': assetsByCategorySumNpv,
                'sumOfAllExpenses': sumOfAllExpenses,
                'expensesSumNpvPerYear': expensesSumNpvPerYear,
                'expensesSumOfAllNPV': expensesSumOfAllNPV, 
                'initialReqLiabilities': initialReqLiabilities, 
                'aggLiabilitiesPerYear': aggLiabilitiesPerYear,
                'aggLiabilitiesPerYearByCategory': aggLiabilitiesPerYearByCategory, 
                'taxPaid': taxPaid, 
                },
        #MUST RETURN:                        
        ...
        #for coverage graph:
            #- sum of income requirements per year for all years
            "incomeRequirementsPerYear": partnerIncomes,
            #- sum of liabilities (lump sum requirements) per year for all years
    ...
    return {
        ...
        #for total requirements graph:
            ...
            "steppedVsLevel": json.dumps(steppedVsLevelTable),
...
def run_local_test():
    ...
    from dummy_data4 import exampleClientThree
    ...
    res = insurance_calc(exampleClientThree, philosophies, assumptions)

    """
    res = lambda_handler({ "body": 
        json.dumps({
            "clientData": exampleClientThree, 
...
if __name__ == "__main__":
    res = run_local_test()    
    #primary = res["primary"]["debugging"]
    #partner = res["partner"]["debugging"]
    #pKeys = partner.keys()
    #keys = primary.keys()
    #print(keys)    